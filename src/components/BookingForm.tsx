...
function BookingForm({ sessionPrice }: BookingFormProps) {
  ...
  const receiptGen = async (id: string) => {
    try {
      ...
      const response = await endpoints.receipts.download(id);
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      const response = await endpoints.bookings.create(submitData);

      if (selectedPlan) {
        await endpoints.admin.notify({
          type: "premium_plan",
          plan: selectedPlan,
          userName: data.name,
        });
      ...
      if (response.data && response.data._id) {
        await receiptGen(response.data._id);
      }
      ...
      // Optionally navigate to dashboard after successful booking
      // navigate("/dashboard");
    } catch (error) {
